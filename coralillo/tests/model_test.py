...
def test_filter(nrm):
    ...
    assert Pet.q().filter(name='bd').one() == pets[2]
...
def test_update_changes_index(nrm):
    ...
    assert debyte_string(nrm.redis.hget(
        'ship:index_code', 'NEWCODE'
    )) == ship.id
...
def test_bounded_model(nrm):
    ...
    assert nrm.redis.hget(
        'testing:tenanted:{}:obj'.format(dev.id), 'name'
    ) == b'foo'
