...
from coralillo.fields import Text, Hash, Bool, Integer, Float, Datetime
from coralillo.fields import Location, Dict
...
from .models import Truck, Subscription
...
def test_field_text():
    ...
    assert field.recover({'field': None}, 'field') is None
    assert field.recover({'field': 'None'}, 'field') is None
...
def test_field_bool():
    ...
    assert field.recover({'field': 'True'}, 'field') is True
    assert field.recover({'field': 'False'}, 'field') is False
    assert field.prepare(True) == 'True'
    assert field.prepare(False) == 'False'
    assert field.recover({'field': None}, 'field') is None
    assert field.recover({'field': 'None'}, 'field') is None

    assert field.validate('true', 'field') is True
    assert field.validate('false', 'field') is False
    assert field.validate('1', 'field') is True
    assert field.validate('0', 'field') is False
    assert field.validate('', 'field') is False
...
def test_field_int():
    ...
    assert field.recover({'field': ''}, 'field') is None
    assert field.recover({'field': None}, 'field') is None
    assert field.recover({'field': 'None'}, 'field') is None
...
def test_field_int_no_required():
    ...
    assert field.validate('', 'field') is None
...
def test_field_date():
    ...
    assert field.recover({'field': '1499794899'}, 'field') == datetime(
        2017, 7, 11, 17, 41, 39
    )
    assert field.recover({'field': ''}, 'field') is None
    assert field.recover({'field': None}, 'field') is None
